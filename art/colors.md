...
- `HSL` - Оттенок, насыщеность, яркость. Удобное представление для человека. HSL
  переводится в RGB по специальным формулам.
- `YCbCr` - Y - яркость, Cb - цвет синий, Cr - цвет красный. Удобная модель
...
В файлах ихображения цветовое пространство определяется при помощи:
...
- `ICC` профиля - встраеваемый файл в файл изображения, который предоставляет
  ...
  преобразовывает теже сырые `RGB` данные в координаты `CIE XYZ` в цветовом
...
в относительных, как в sRGB. Чаще всего работает в `Rec.2020` цветовом
...
В видео и изображениях часто данные храняться в моделе `YCbCr` (яркость, синий и
красные каналы). Это связанно с тем что люди более воспреимчивы к изменению
яркости, чем к изменению цвета. Такой способ представления цвета позволяет
эффективно сжимать с потерей информации.

Но цветовые пространства чаще всего определены в рамках `RGB` модели. Поэтому
...
> ⚠️ Внимание: В иллюстрации для наглядности берутся цвета перых пикселей, а не
смешанные цвета.


Субдискретизация в основном сжимает сэмплы внутри блоков 2x2 пикселя. Но
обратите внимение, что `4:1:1` работает подругому и сжимает полоски 4x1.
